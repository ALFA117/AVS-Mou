  ...
  return (
    <main className="mx-auto max-w-4xl px-6 py-10">
      <div className="flex items-center justify-between">
        <h1 className="font-heading text-2xl font-bold tracking-tight text-foreground">{t("dashboard.title")}</h1>
      </div>
      ...
      {!publicKey ? (
        ...
        <>
          ...
          <div className="mt-8 flex items-center justify-between">
            <h2 className="font-heading text-lg font-semibold tracking-tight text-foreground">{t("dashboard.positions")}</h2>
            {positions.length > 0 && (
              <button
                  ...
                  >
                    <div>
                      ...
                      <p className="font-mono-avs text-xs text-muted-foreground">
                        {t("dashboard.bidLine", {
                          amount: formatTokenAmount(p.bidAmount),
                          equity:
                            Number(p.equityAllocated) > 0
                              ? formatEquity(p.equityAllocated)
                              : t("dashboard.pending"),
                        })}
                      </p>
                    </div>
